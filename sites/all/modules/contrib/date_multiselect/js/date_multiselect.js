    ...
    attach: function (context) {
      for (var id in Drupal.settings.dateMultiselect) {
        ...
        if (!input.hasClass('date-multiselect-init')) {
          ...
          if (settings['input']) {
            input.addClass('date-multiselect-init').multiDatesPicker(settings);
          }else  {
            input.addClass('date-multiselect-init').wrap('<div id="' + id + '-wrapper" />').parent().multiDatesPicker(settings);
            input.hide();
          }
